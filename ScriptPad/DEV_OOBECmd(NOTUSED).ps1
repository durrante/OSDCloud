## This script triggers OOBEDeploy.cmd upon first boot.

$scriptFolderPath = "$env:ProgramData\OSDeploy\Custom_Scripts\"
$ScriptPathOOBE = $(Join-Path -Path $env:SystemDrive\Windows\ -ChildPath "OOBEDeploy.cmd")
...
Start-Transcript -Path (Join-Path "`$env:ProgramData\OSDeploy\Custom_Scripts\" `$Global:Transcript) -ErrorAction Ignore | Out-Null
...
Invoke-WebRequest https://github.com/durrante/OSDCloud/raw/main/ServiceUI64.exe -OutFile "C:\OSDCloud\ServiceUI.exe"
